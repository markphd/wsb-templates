...
db.find({"pages.id": { $in: liveTemplates } }, (err, templates) => findTemplateOrder(templates))

function findTemplateOrder(templates) {
  let templateOrder = []
  templates.filter( item => {
  	// console.log(Object.values(item.pages))
    let templatePages = Object.values(item.pages);
    for(const value of templatePages){
      if (liveTemplates.indexOf(value.id) >= 0) {
        templateOrder.push({index: liveTemplates.indexOf(value.id), id: item.id, pageId: value.id, name: item.name, categories: item.categories})
      } else {
      // console.log(liveTemplates.indexOf(value.id), value.id, item.name, item.categories)
      }
